...
export function ChatPanel({ messages, onSendMessage, onSendFile, isOpen, onClose }: ChatPanelProps) {
    ...
    return (
        <>
            ...
            <div
                className={cn(
                    "bg-white border-l border-border flex flex-col transition-all duration-300 overflow-hidden z-50",
                    // Mobile: slide up from bottom as full overlay
                    "fixed inset-x-0 bottom-0 md:static md:inset-auto",
                    isOpen
                        ? "h-[85dvh] md:h-full md:w-80 rounded-t-2xl md:rounded-none"
                        ...
                        <div className="p-3 border-t border-border pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="w-10 h-10 rounded-full flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer flex-shrink-0"
                                    title="Send a file"
                                >
                                ...
                                <input
                                    ...
                                    className="flex-1 h-10 px-3 text-sm bg-muted rounded-lg border-none outline-none focus:ring-2 focus:ring-primary/30 min-w-0"
                                />
                                <button
                                    onClick={handleSend}
                                    disabled={!text.trim()}
                                    className="w-10 h-10 rounded-full flex items-center justify-center text-primary hover:bg-primary/10 disabled:opacity-30 transition-colors cursor-pointer flex-shrink-0"
                                >
